...
class TestRequestMailNotificationLinks(AccessRulesFixMixinMT, HttpCase):
    ...
    def test_assign_employee(self):
        request = self.env['request.request'].with_context(
            ...
            'type_id': self.env.ref(
                'dgf_request_base.request_type_sequence').id,
            'request_text': 'Test',
        })
    ...
    def test_change_employee(self):
        ...
        request.message_subscribe(
            ...
            subtype_ids=(
                ...
                self.env.ref(
                    'dgf_request_base.mt_request_stage_changed')
            ).ids)
        self.flush_tracking()
        with disable_mail_auto_delete(self.env):
    ...
    def test_subrequest_event_message(self):
        ...
        simple_type = self.env.ref('dgf_request_base.request_type_simple')

        # create parent request
        parent_request = Request.create({
            'type_id': simple_type.id,
            'category_id': self.env.ref(
                'dgf_request_base.request_category_demo_general').id,
        ...
        child_request.write({
            'stage_id': self.env.ref(
                'dgf_request_base.request_stage_type_simple_sent').id
        })
        ...
        with FindNew(self.env, 'mail.message') as nr:
            child_request.write({
                'stage_id': self.env.ref(
                    'dgf_request_base.request_stage_type_simple_rejected').id
            })
        ...
        with FindNew(self.env, 'mail.message') as nr:
            child_request.write({
                'stage_id': self.env.ref(
                    'dgf_request_base.request_stage_type_simple_draft').id
            })
