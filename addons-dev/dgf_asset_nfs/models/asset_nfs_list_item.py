...
class AssetNFSListItem(models.Model):
    ...
    company_mfo = fields.Char(string="МФО банку", related="asset_nfs_list_id.company_id.mfo", store=True, readonly=True)
    ...
    book_value = fields.Monetary(string='Балансова вартість, номінал', currency_field='currency_id', tracking=True)    
    book_value_uah = fields.Float(string='Балансова вартість, UAH', digits=(15, 2), tracking=True)
    apprisal_value = fields.Float(string='Оціночна вартість, UAH', digits=(15, 2), tracking=True)
    reason_documents = fields.Char(string='Підтвердні документи', tracking=True)
    note = fields.Char(string='Примітки', tracking=True)
