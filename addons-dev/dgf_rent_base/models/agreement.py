...
class Agreement(models.Model):
    ...
    asset_ids = fields.One2many(string="Активи лоту", related="procedure_lot_id.asset_ids",)
                                #   comodel_name='dgf.procedure.lot.asset',
                                #   inverse_name='lot_id')
    asset_ids_count = fields.Integer(string="Кількість активів", related="procedure_lot_id.asset_ids_count", readonly=True, store=True)
    ...
    # Internal Methods
    # ----------------------------------------
    def create_lot_action(self):
        for record in records:
            if not record.procedure_lot_id:
                create_values = {
                'lot_id': record.code,
                'name': record.notes,
                'category_id': env.ref('dgf_rent_base.rent_expense_lot').id, # замінити на змінну
                'company_id': record.company_id.id,
                'contragent_id': record.partner_id.id if record.partner_id else False,
                }
                lot = env['dgf.procedure.lot'].create(create_values)
                if lot:
                    write_values = {'procedure_lot_id': lot.id}
