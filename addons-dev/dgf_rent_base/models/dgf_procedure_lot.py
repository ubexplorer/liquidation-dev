...
class DgfProcedureLot(models.Model):
    ...
    company_id = fields.Many2one('res.company', string='Банк', readonly=False)
    # TODO: change logic in auctions modules: replace partner_id -> company_id
    contragent_id = fields.Many2one('res.partner', string='Контрагент')
    lot_type = fields.Selection(
        selection_add=[
            ('rent_income', 'Лот з оренди (дохідна)'),
            ('rent_expense', 'Лот з оренди (витратна)'),
            ],
        ondelete={'rent_income': 'set null', 'rent_expense': 'set null'})
    asset_ids = fields.One2many(string="Активи лоту",
                                  comodel_name='dgf.procedure.lot.asset',
                                  inverse_name='lot_id')
    asset_ids_count = fields.Integer(string="Кількість активів", compute='_compute_asset_ids_count', store=True)
    ...
    def _compute_asset_ids_count(self):
        for record in self:
            record.asset_ids_count = len(record.asset_ids)

    # ----------------------------------------
    # Actions Methods
    # ----------------------------------------
    def lot_asset_action(self):
        return {
            ...
            'context': {
                ...
                'default_company_id': self.partner_id.company_ids[0].id,
