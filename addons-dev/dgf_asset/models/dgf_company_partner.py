...
class CompanyPartner(models.Model):
    ...
    name = fields.Char(related='partner_id.name', string='Company Partner Name', required=True, store=True, readonly=False)
    sequence = fields.Integer(help='Used to order Companies in the company switcher', default=10)
    parent_id = fields.Many2one('dgf.company.partner', string='Parent Company', index=True)
    child_ids = fields.One2many('dgf.company.partner', 'parent_id', string='Child Companies')
    ...
    def create(self, vals):
        ...
        # think if this block needed?
        if not vals.get('name') or vals.get('partner_id'):
            self.clear_caches()
            return super(CompanyPartner, self).create(vals)
        # TODO: check if partner exist
        partner_record = self.env['res.partner'].sudo().search([('vat', '=', vals.get('vat'))])
        if not partner_record.exists():
            partner = self.env['res.partner'].create({
                'name': vals['name'],
                'is_company': vals.get('is_company'), # переробити для ЮО та ФО
            ...
            vals['partner_id'] = partner_record.id  # error: remake

        self.clear_caches()
        company = super(CompanyPartner, self).create(vals)
    ...
    def write(self, values):
        ...
        res = super(CompanyPartner, self).write(values)
