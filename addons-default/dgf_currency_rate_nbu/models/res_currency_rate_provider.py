...
class ResCurrencyRateProvider(models.Model):
    ...
    interval_type = fields.Selection(
        ...
        selection=[("days", "Day(s)"), ("weeks", "Week(s)"),
                   ("months", "Month(s)")],
        default="days",
        required=True,
    )
    ...
    def _update(self, date_from, date_to, is_period, newest_only=False):
        Currency = self.env["res.currency"]
        ...
        for provider in self:
            ...
            # - remove company_id if country_id = Ukraine
            if provider.company_id.country_id.id == self.env.ref('base.ua').id:
                company_value = None
            else:
                company_value = provider.company_id.id

            # - if is_period=True - use cursor in separate method
            if is_period is True and company_value is None:
                provider._process_period_rates(provider.currency_ids.mapped("name"), data, provider.id)
            else:
                for content_date, rates in data:
                    ...
                    for currency_name, rate in rates.items():
                        ...
                        if record:
                            record.write(
                                {"rate": rate, "provider_id": provider.id})
                        else:
            ...
            provider.message_post(
                subject=_("Currency Rate Update"),
                channel_ids=[1],
                body=_(
                    'Currency Rate Updated: Provider "%s" '
                    " since %s until %s"
                )
                % (
                    provider.name,
    ...
    def _schedule_next_run(self):
        ...
        self.next_run = (
            datetime.combine(self.next_run, time.min) +
    ...
    def _process_rate(self, currency, rate):
        ...
        value = direct
        if (
            currency_rate_inverted and currency.with_context(
    ...
    def _scheduled_update(self):
        ...
        if providers:
            ...
            for provider in providers.with_context({"scheduled": True}):
                ...
                # newest_only=True
                provider._update(date_from, date_to, is_period=False, newest_only=False)

        msg = _("Scheduled currency rates update complete.")
        _logger.info(msg)
        return msg
