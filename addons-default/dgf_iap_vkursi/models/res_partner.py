...
class Partner(models.Model):
    ...
    def getorganizations(self):
        ...
        response = self.env['vkursi.api']._api_getorganizations(
    ...
    def getadvancedorganization(self):
        provider_name = self.env['iap.account'].get('vkursi')._provider_name
        response = self.env['vkursi.api']._api_getadvancedorganization(
            code=self.vat, description=provider_name)
        print(response)
        #  analyse errors with responce
        # json_data = json.loads(responce)
    ...
    def gethttpbin(self):
        provider_name = self.env['iap.account'].get('vkursi')._provider_name
        response = self.env['vkursi.api']._api_httpbin(description=provider_name)
        print(response)
        # handle {'status_code': 200, 'result': 'Not found'}
        request_result = response['request_result']
        request_datetime = response['request_datetime']
