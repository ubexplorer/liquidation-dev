...
class AssetNfsRequest(models.Model):
    ...
    # TODO: змінити <field name="description"></field> в base_type.xml
    ...
    def _compute_request_totals(self):
        for record in self:
            if record.type_code == 'exclude':
                record.request_total_book_value_uah = sum(item.book_value_uah for item in record.asset_nfs_exclude_ids)
                record.request_total_apprisal_value = sum(item.apprisal_value for item in record.asset_nfs_exclude_ids)
            else:                
                record.request_total_book_value_uah = sum(item.book_value_uah for item in record.asset_nfs_ids)
                record.request_total_apprisal_value = sum(item.apprisal_value for item in record.asset_nfs_ids)            
