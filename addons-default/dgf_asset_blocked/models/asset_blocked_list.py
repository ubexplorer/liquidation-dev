...
class AssetBlockedList(models.Model):
    ...
    _list_name_template = "Перелік майна {}, що не підлягає продажу (частина з обмеженим доступом)"

    def _compute_item_count(self):
        for item in self:
            item.item_count = len(item.asset_blocked_ids)
            item.item_count_active = len(item.asset_blocked_ids.filtered(lambda x: x.is_closed is False))
    ...
    def _compute_totals(self):
        for record in self:
                record.total_book_value_uah = sum(item.book_value_uah for item in record.asset_blocked_ids.filtered(lambda x: x.is_closed is False))
                record.total_apprisal_value = sum(item.apprisal_value for item in record.asset_blocked_ids.filtered(lambda x: x.is_closed is False))
