...
class Agreement(models.Model):
    ...
    agreement_item_count = fields.Integer(string="Кількість майна відчуженого", compute='_compute_agreement_item_count', store=False, readonly=True)
    ...
    asset_blocked_dz_id = fields.Many2one('asset.blocked.list.item', string="ДЗ перекласифікована", ondelete='restrict', index=True)
    ...
    type_code = fields.Char(string='Код типу', related='type_id.code', readonly=True)
    ...
    def _onchange_type_id(self):
        for record in self:
            if record.type_id.code in ['freeuse', 'transport']:
                record.domain = 'free'
            else:
                record.domain = 'sale'
        ...
        for record in self:
            if new_stage_id.code == 'approved':
                if len(record.asset_blocked_ids.ids) == 0:
                    ...
                    # TODO: для майна примусово відчуженого визначити відповідний статус замість "передано"
    ...
    def set_parent_item_action(self):
        self.ensure_one()
        self.asset_blocked_linked_ids.sudo().write({'parent_id': self.asset_blocked_dz_id.id})


    # def agreement_list_item_dz_action(self):
    #     return {
    #         'type': 'ir.actions.act_window',
    #         'name': 'Майно в документі',
    #         'view_type': 'form',
    ...
    #         'res_model': 'asset.blocked.list.item',
    #         'view_id': False,
    #         'domain': [('agreement_dz_id', '=', self.id)],
    #         'context': {
    #             'default_agreement_dz_id': self.id,
    #         },
    #     }
