...
class MailActivity(models.Model):
    ...
    # dgf_document_id = fields.Many2one('dgf.document.blocked', string='Документ', store=True, readonly=True, compute='_get_doc_id')

    @api.depends('res_model', 'res_id')
    def _get_doc_id(self):
        for record in self:
            if record.res_model == 'dgf.document.blocked':
