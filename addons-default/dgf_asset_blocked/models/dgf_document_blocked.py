...
class DgfDocument(models.Model):
    _name = 'dgf.document.blocked'
    _description = 'Внутрішній документ з обмеженим доступом'
    ...
    parent_document_id = fields.Many2one('dgf.document.blocked', string="Головний документ", ondelete='restrict', index=True)
    child_ids = fields.One2many('dgf.document.blocked', 'parent_document_id', string="Похідні документи", index=True)  # ondelete='restrict',
    ...
    description = fields.Text(string='Реквізити ДЗ')
    notes = fields.Text(string='Примітки')
    document_private_content = fields.Html(string='Зміст документа непублічний')
    document_file = fields.Binary(string="Образ документа", attachment=True)  # attachment=False
    file_name = fields.Char("І'мя файлу")
    partner_ids = fields.Many2many('res.partner', 'dgf_doc_blocked_res_partner_rel', 'doc_id', 'partner_id', string='Банки', required=True, domain="[('is_company','=',True)]")
    is_public = fields.Boolean(string="Публічний", default=False, groups='dgf_document.group_documents_private')
    state = fields.Selection(
        [("draft", "Чернетка"), ("approved", "Затверджено"), ("canceled", "Втратив чинність")],
    ...
    def action_create_from_parent(self):
        ...
        return {
            'name': 'Документи',
            'view_type': 'form',
            'res_model': 'dgf.document.blocked',
            'view_id': False,
            'view_mode': 'form',
            # 'target': 'new',
            'context': {
