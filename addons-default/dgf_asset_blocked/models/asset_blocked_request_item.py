...
class AssetBlockedRequestItem(models.Model):
    ...
    asset_blocked_list_item_id = fields.Many2one('asset.blocked.list.item', string="Майно не для продажу", ondelete='restrict', index=True, copy=False)
    ...
    _sql_constraints = [
        ('request_item_uniq', 'unique(request_id,asset_blocked_list_item_id)', "Майно з таким кодом вже існує в цій заявці.")
    ]


    @api.model
    def create(self, vals):
        if any(['asset_blocked_list_item_id' not in vals.keys(), vals.get('asset_blocked_list_item_id') is False]):
        # if 'asset_blocked_list_item_id' not in vals.keys():
        ...
        # print("asset_blocked_list_item_id = {}".format(vals['asset_blocked_list_item_id']))
