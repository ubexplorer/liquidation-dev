...
def _json_response(self, result=None, error=None):
    ...
    if self.endpoint and self.endpoint.routing.get('kw_type') == 'http':
        return result
...
class JsonRequestNew(http.JsonRequest):
    def __init__(self, *args):
        ...
        r = self.httprequest.get_data().decode(self.httprequest.charset)

        try:
            self.jsonrequest = json.loads(r)
        except ValueError:
            self.jsonrequest = {}
