...
class AuthController(http.Controller):
    ...
    def kw_api_auth_token_post(self, kw_api, **kw):
        ...
        token.update_token(both=True)
        return kw_api.data_response(token.kw_api_get_record_value())
    ...
    def kw_api_auth_token_refresh_post(self, kw_api, **kw):
        ...
        token = request.env['kw.api.token'].sudo(
            ).refresh_token_by_refresh_token(token)
        return kw_api.data_response(token.kw_api_get_record_value())
